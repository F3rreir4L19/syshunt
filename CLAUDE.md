...
| 2026-05-13 | `run_full_pipeline` usa `apply_async(link=run_http_probe.si(...).set(link=run_nuclei_scan.si(...)))` em vez de `chain().apply_async()` interno | Consistência com padrão Celery de chaining via link=; funciona corretamente em modo eager |
